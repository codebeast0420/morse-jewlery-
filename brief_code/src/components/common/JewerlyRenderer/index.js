...
class JewerlyRingsRenderer extends React.Component {
  ...
  getImage() {
    ...
    const base64 = this.renderer.getWebGlRenderer().domElement.toDataURL("image/png", 1.0);
  ...
  componentDidMount() {
    ...
    self.renderer = new Renderer(self.webgldrawarea_id, self.webgldrawarea_size_allocator_id, function onReadyToUse() {
  ...
  update(prevProps, prevState, bForce) {
    ...
    if (self.renderer) {
      ...
      if (prevProps && (prevProps.chainLength !== self.props.chainLength || prevProps.createChainType !== self.props.createChainType || !bIsEqualsArray)) {
      ...
      if (bForce || prevProps.createBangle !== self.props.createBangle || prevProps.chainLength !== self.props.chainLength || prevProps.createChainType !== self.props.createChainType || !bIsEqualsArray) {
...
/*
    return (
      <div style={{ width: "100%", height: "100%" }} className="canvas">
        <canvas id={webgldrawarea_size_allocator_id} style={{ width: "100%", height: "100%", opacity: 0 }}/>
        <canvas id={webgldrawarea_id} style={{ display: "none", opacity: 0 }}/>
      </div>
    );*/
  }
  ...
  resetCamera(chainType) {
    ...
    if (chainType) {
      if (chainType === "NECKLACE" || chainType === "BRACELET" || chainType === "PENDANT") {
  ...
  uploadRingsByUrls() {
    ...
    function onPromisesDone(gltfs) {
      if (this.ignore) {
        // this is uploadRingsByUrls.promise
        return;
      }
    ...
    self.uploadRingsByUrls.uploadGLTF_promises.then(onPromisesDone.bind(self.uploadRingsByUrls.uploadGLTF_promises));
  }
  ...
  createBangleByUrls(bangleOptions) {
      ...
      if (gltfs.length) {
        ...
        gltfs.forEach((bangleArray) => {
          ...
          bangleGltf.traverse((object) => {
            if (!bangleGltf.radiusA && object.name.toUpperCase().search("RADIUSA") > -1) {
              //
              bangleGltf.radiusA = object;
            }
            if (!bangleGltf.radiusB && object.name.toUpperCase().search("RADIUSB") > -1) {
              //
          ...
          function setElement(object, t) {
            ...
            node.lookAt(new THREE.Vector3(curve_point.x + 10.0 * curve_tangent.x, curve_point.y + 10.0 * curve_tangent.y, 0));
          ...
          function proc(initial_T, setElement) {
            let tLeft = initial_T;
            ...
            if (true) {
              ...
              for (let k1 = dashArrays_Index + 1; k1 < dotDashArrays.length; k1++) {
                // letters array
                ...
                for (let k_i = arr.length - 1; k_i >= 0; k_i--) {
                  // dashes and dots array
          ...
          const procResult = proc(0.5, () => {});

          if (true) {
            const nextLength = curve_length / 2 - (procResult.fullLengthLeft + procResult.fullLengthRight) / 2 + procResult.fullLengthLeft;
    ...
    self.createBangleByUrls.uploadGLTF_promises.then(onPromisesDone.bind(self.createBangleByUrls.uploadGLTF_promises));
  }
  ...
  createChainByUrls(chainType) {
      ...
      clone.traverse((object) => {
        ...
        if (object.material && object.material.isDiamondMaterial) {
          function applyDiamondUniformOverride(diamondUniformOverrides, object, name) {
            if (diamondUniformOverrides.hasOwnProperty(name) && object.material.uniforms.hasOwnProperty(name)) {
    ...
    function create_BRACELET_NECKLACE() {
      ...
      if (Array.isArray(self.props.ringsUrls[0]) || typeof self.props.ringsUrls[0] !== "object") {
        return null;
      }
      ...
      function onPromisesDone(gltfs_float_array) {
        if (this.ignore) {
          // this is createChainByUrls.promise
          return;
        }
        ...
        if (gltfs.length > 1) {
          ...
          if (bUseSpline && chainType !== "BRACELET") {
            ...
            if (json) {
              ...
              const splineEditor = new SplineEditor(null, scene, null, null);
          ...
          for (let i = 0; i < gltfs.length; i++) {
            ...
            if (bApplyCustomSpline) {
              ...
              for (let j = 1; j < slineHelperObjects.length; j++) {
                ...
                if (j > 0) {
                  ...
                  if (prev_t <= mpT && helper.t >= mpT) {
                    ...
                    additional_rotation = prev_rotation * Math.abs(-0.5 + c) + helper.rotation.x * (0.5 + c);
                  }
                }
              }
              ...
              const vFrom = new THREE.Vector3().subVectors(point2, node.position).normalize();

              const vTo = new THREE.Vector3().subVectors(curve_point2, curve_point1).normalize();
    ...
    if (!onPromisesDone || !self.createChainByUrls.uploadGLTF_promises) {
      return;
    }

    self.createChainByUrls.uploadGLTF_promises.then(onPromisesDone.bind(self.createChainByUrls.uploadGLTF_promises)).then(uploadBust.bind(self.createChainByUrls.uploadGLTF_promises)).then(adjustCamera.bind(self.createChainByUrls.uploadGLTF_promises));
  }
...
const Renderer = function (canvas_id, canvas_size_allocator_id, onReadyToUse) {
  "use strict";
  ...
  function getStoneMaterialUniform(uniform, handle) {
    if (scene) {
      scene.traverse(function (object) {
        if (object.isMesh) {
          if (object.name.toUpperCase().search("DIAMOND") > -1 || object.name.toUpperCase().search("STONE") > -1) {
  ...
  function setRingMaterialValue(valueName, value) {
    if (scene) {
      scene.traverse(function (object) {
        if (object.isMesh) {
          if (object.name.toUpperCase().search("RING") > -1 || object.name.toUpperCase().search("METAL") > -1) {
  ...
  function Component3d() {
    ...
    function init() {
      ...
      self.uploadGLTF = (url, onBeforeLoad, onLoad, onProgress, forceSetName, forceUpload) => {
          ...
          if (!gltf.done) {
            gltf.traverse(function (object) {
              if (object.isMesh) {
                ...
                if (pmremCubeUVPacker && object.name.toUpperCase().search("DIAMOND") === -1 && object.name.toUpperCase().search("STONE") === -1) {
        ...
        for (let id in diamondLoaders) {
          ...
          for (let i = 0; i < diamondLoader.diamonds.length; i++) {
            ...
            for (let j = 0; j < diamond.sparkles.length; j++) {
              ...
              sparkle.material.uniforms.screenTexture.value = elPIXEL.getEffectComposer().renderTarget2.texture;
            }
          }
        }
      } else {
...
function isEqualsArray(array1, array2) {
  ...
  for (var i = 0, l = array1.length; i < l; i++) {
    ...
    if (array1[i] instanceof Array && array2[i] instanceof Array) {
      ...
      if (!isEqualsArray(array1[i], array2[i])) return false;
