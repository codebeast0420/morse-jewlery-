...
import { getProductAction, setProductConfigurationAction, setSavedAction, setProductAction, setProductPArtsAction } from "../../../store/actions/ProductsActions";
...
class MessageModal extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.text === this.state.text && prevState.textTwo === this.state.textTwo && prevProps.product.messageInputs === 2) {
      this.animation();
    }
  }
  ...
  render() {
    const { onClose, product, dispatchGetProduct, configuration } = this.props;
    const { html, htmlTwo, text: message, placeholderVisibility, inputFocused, textTwo, placeholderTwoVisibility, inputFocusedTwo, tooBig, bangleToBig } = this.state;
    ...
    return (
      {/*
      <Modal {...this.props} style={customStyles} closeOnEsc={false} closeOnOverlayClick={false} closeTimeoutMS={300}>
        <Header
          onProd
          onClose={() => {
            ...
            const { changingProduct, dispatchSetConfiguration, dispatchSetProduct, dispatchSetProductParts } = this.props;
            if (changingProduct) {
        ...
        <div className="message container">
          <div className="message__inputs">
            <div className={`message__input-container ${product.messageInputs === 2 ? "two" : ""}`}>
              <div className="morse-code">{this.buildMorseCode(this.state.text)}</div>
              <ContentEditable
                html={html}
                disabled={false}
                onChange={(e) => {
              ...
              />
              {product.messageInputs === 1 && <div className={`message__placeholder ${visible}`}>Type your message</div>}
              {product.messageInputs === 2 && <div className={`message__placeholder ${visible}`}>Left</div>}
            </div>
            {product.messageInputs === 2 && (
              <div className="message__input-container two">
                <div className="morse-code">{this.buildMorseCode(this.state.textTwo)}</div>
                <ContentEditable
                  html={htmlTwo}
                  disabled={false}
                  onChange={(e) => {
                  ...
                  className="message__input"
                />
                {product.messageInputs === 2 && <div className={`message__placeholder ${visibleTwo}`}>Right</div>}
              </div>
            )}
          </div>
            ...
            <CSSTransition in={tooBig} timeout={500} classNames="too-big">
              <div className="message__maxCharactersText size" style={{ position: "absolute", top: 0, width: "100%" }}>
                {_.isEmpty(bangleToBig) && (
                  <div>
                    Your chosen message exceeds our maximum necklace length of 75cm. Please send us the requested message to <a href="mailto:hello@codebyedge.com">hello@codebyedge.com</a> and one of customer care team members will be in contact.
                  </div>
                )}
                {!_.isEmpty(bangleToBig) && (
                  <div>
                    Your {sentence} exceeds our maximum bangle length. Please send us the requested message to <a href="mailto:hello@codebyedge.com">hello@codebyedge.com</a> and one of customer care team members will be in contact.
                  </div>
                )}
              </div>
            </CSSTransition>
          ...
          <div className="message__tips">
            <div className="message__tips__title">Need inspiration?</div>
            <div className="message__tips__examples">{product.inspiration && product.inspiration.map((value) => this.getTip(value.message, value.description))}</div>
          </div>
        </div>
