...
import { faAngleDoubleRight, faCircleNotch } from "@fortawesome/free-solid-svg-icons";
import imageCompression from "browser-image-compression";
import { Logo, Back, Close, Dots, Ring, Share, Bookmark, Message, Bag } from "../../../assets";
import OptionsModal from "../OptionsModal";
import InfoModal from "../InfoModal";
import ShareModal from "../ShareModal";
import SaveDesignModal from "../SaveDesignModal";
import { URL, client } from "../../../library/env";
import NotSavedModal from "../NotSavedModal";
import DoneModal from "../DoneModal";
import { setDoneModalAction, changingProductAction, setProductAction, setProductConfigurationAction } from "../../../store/actions";
...
class Header extends React.PureComponent {
  ...
  handleClick = async () => {
    const { product, configuration, imageFunc, productParts, queryString, preMadeProduct } = this.props;
    await imageCompression
      .getFilefromDataUrl(imageFunc())
      .then(async (file) => {
        const options = {
  ...
  modals = () => {
    ...
    };
    {
      /*
              ...
              <div className="options__content__menu__item" onClick={() => {
              ...
              >
                ...
                <div className="options__content__menu__item__icon"><Share></Share></div>
              </div>
              {!preMadeProduct.isPremade && (
                <div className="options__content__menu__item" onClick={() => this.setState({ options: false, saveDesign: true, fromDone: false })} onKeyUp={() => false} role="button" tabIndex={0}>
    ...
    }
    {
      /*<div className="options__content__menu__item__icon"><Bookmark /></div>
              ...
              <div className="options__content__menu__item" onClick={() => this.setState({ info: true, options: false, fromDone: false })} onKeyUp={() => false} role="button" tabIndex={0}>
                <div className="options__content__menu__item__title">Talk to us</div>
                <div className="options__content__menu__item__icon">
                  <Message />
                </div>
              </div>
            </div>
    ...
    const doneModal = () => {
      ...
      /*
      return (
        <DoneModal {...this.props} open={modal} onClose={() => dispatchSetModal(false)}>
          <div className="done__content">
            <div className="done__content__menu">
              ...
              >
                ...
                <div className="done__content__menu__item__icon">
                  <Share />
                </div>
              </div>
              {!preMadeProduct.isPremade && (
              ...
              >
                <div className="save-design__content__button__content">
                  <div className="save-design__content__button__title">Purchase my design</div>
                  <div className="save-design__content__button__icon">{(addToCart && <FontAwesomeIcon icon={faCircleNotch} className="social-link" spin />) || <Bag />}</div>
                </div>
              </ClickableDiv>
            </div>
          </div>
          ...
          <Row>
            ...
            <Col className="header__item">
              ...
              {pathname === "/product" && !onProd && (
                <div className="header__button" onClick={() => this.setState({ options: true })} onKeyUp={() => {}} role="button" tabIndex={0}>
                  <Dots />
                </div>
              )}
            </Col>
