    ...
    return (
      ...
      <div className="gallery-modal__close">
        ...
        <div className="gallery-modal__photos">
          <div className="gallery-modal__photos__title"/>
          <div className="gallery-modal__photos__top">
            
            <div className="swiper-wrapper">
              {product.data.gallery.map((image) => (
                <div className="swiper-slide gallery-modal__photos--image" key={Math.random() * 1000} style={{ backgroundImage: `url(${image})` }} />
              ))}
              </div>
          </div>
          <div className="gallery-modal__photos__thumbs">
            <div className="swiper-wrapper">
              {product.data.gallery.map((image) => (
                <div className="swiper-slide gallery-modal__photos--thumb" key={Math.random() * 1000} style={{ backgroundImage: `url(${image})` }} />
              ))}
            </div>
          </div>
        </div>
