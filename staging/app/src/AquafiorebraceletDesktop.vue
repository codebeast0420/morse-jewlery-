  ...
  <div class="w-full">
    ...
    <div class="flex flex-row content-center justify-center mt-6">
      ...
      <div class="basis-6/12 flex items-center" style="display: flex; justify-content: center">
        <button class="bg-gray-100 hover:bg-gray-200 cbe-btn-text-green cbe-btn-text-font py-2 px-12 rounded-none h-3/4"
          @click="$parent.modalVisible = true">
      ...
      <div class="basis-5/12 flex pr-1" style="display: flex; justify-content: end">
        ...
        <div class="basis-9/12 flex justify-center">
          <button v-if="!($parent.working)"
            :class="`${$parent.buyButtonDisabled ? 'bg-slate-500' : 'cbe-bg-green hover:bg-teal-700'} w-full py-8 px-4 rounded-lg justify-self-end text-2xl font-extrabold text-white`"
            :disabled="$parent.buyButtonDisabled" @click="$parent.addToCart">
    ...
    <div class="flex flex-row content-center justify-center">
      ...
      <div class="basis-6/12" style="display: flex; justify-content: center">
        ...
        <!-- Amanti Rings-->
        <div v-if="$parent.key == 'aquafiore-bracelet'">
          <div class="cbe-canvas mt-12">
            <Renderer :resize="true" :orbitCtrl="true" :antialias="true" ref="renderer" :alpha="true">
              ...
              <Scene ref="scene" background="white">
                <GltfModel v-for="(glb, index) in $parent.glbArray" :key="index"
                  :src="`/static/src/glb/ring_` + glb + `.glb`" @load="onReady"
                  :position="{ x: index * 0, y: index * 0, z: index * 1.5 - 2 }" :rotation="{ x: 0, y: 0, z: 0 }"
                 />
              </Scene>
              <EffectComposer>
                <RenderPass />
        ...
        <div class="basis-9/12 flex flex-col">
          ...
          <div class="flex flex-row" style="display: flex; justify-content: center">
            ...
            <div class="basis-4/12 pr-1">
              <button
                :class="`${$parent.optionActive == 'message' ? 'cbe-bg-green-lightest' : 'bg-gray-100 hover:bg-gray-200'} cbe-btn-text-green cbe-btn-text-font py-6 w-full rounded-none`"
                @click="$parent.rowUpdate('message')">
            ...
            <div class="basis-4/12 pr-1">
              <button
                :class="`${$parent.optionActive == 'metal' ? 'cbe-bg-green-lightest' : 'bg-gray-100 hover:bg-gray-200'} cbe-btn-text-green cbe-btn-text-font py-4 w-full rounded-none`"
                @click="$parent.rowUpdate('metal')">
                <div class="flex flex-row content-center justify-center gap-x-2">
            ...
            <div class="basis-4/12">
              <button
                :class="`${$parent.optionActive == 'size' ? 'cbe-bg-green-lightest' : 'bg-gray-100 hover:bg-gray-200'} cbe-btn-text-green cbe-btn-text-font py-4 w-full rounded-none`"
                @click="$parent.rowUpdate('size')">
                <div class="flex flex-row content-center justify-center gap-x-2">
                  <div class="basis-3/4 flex flex-col">
                    <p class="italic text-xs">Select</p>
          ...
          <div class="flex flex-row mt-4" style="display: flex; justify-content: center">
            ...
            <div v-show="$parent.optionActive == 'message'" :class="`basis-11/12`"
              v-if="$parent.classes.amanti.includes($parent.key) && $parent.key != 'aquafiore-pendant'">
              <div class="message__inputs">
                <div class="message__input-container">
                  <div class="morse-code" id="morseContainer"></div>
                  <div class="message__input cbe-font-mono" :contenteditable="$parent.editableAmanti" @input="updateMorse"
                    id="messageInput" :onkeypress="$parent.amantiValidateInput"
                    :style="{ 'font-size': $parent.fontSize + 'rem !important', 'letter-spacing': $parent.lineSpacing + 'rem' }">
                  </div>
                  <div
                    class="message__placeholder message__placeholder--visible cbe-font cbe-message-placeholder-fix mt-1">
                    {{ $parent.placeholder }}</div>
                </div>
              </div>
              <div v-if="$parent.isRendering" class="col-span-3 align-middle grid grid-cols-3 content-center">
                <div></div>
                <div class="flex justify-center"><img class="text-center align-middle" src="./Infinity-1s-197px.gif" />
                </div>
                <div></div>
              </div>
              <div v-if="!$parent.isRendering" class="col-span-3 align-middle">
                <p class="text-center text-stone-500 cbe-font-label mt-1 text-sm">Zoom: Mouse Wheel, Rotate: Left Mouse
                  Button, Pan: Right Mouse Button</p>
              </div>
              <div></div>
            </div>
            <!-- Select Metal-->
            <div v-show="$parent.optionActive == 'metal'" :class="`flex flex-col basis-12/12 w-full`"
              style="display: flex; justify-content: center">
              <div class="mb-1" v-for="option in $parent.obj.options" :key="option">
                <button
                  :class="`${$parent.isActiveFinish(option.text) ? 'cbe-bg-green-lightest' : 'bg-gray-100 hover:bg-gray-200'} py-6  rounded-none w-full py-8 px-10 rounded-lg justify-self-end`"
                  :disabled="$parent.isActiveFinish(option.text)" @click="updateFinish(option.text)">
                  <div class="flex justify-between px-8" style="display: flex; justify-content: space-between">
              ...
              <div class="cbe-font italic cbe-btn-text-green py-2">
                <p class="leading-6 font-semibold">Our precious metals and gemstones are all sourced in accordance with
                  the Responsible Jewellery Council Code of Conduct</p>
              </div>
            </div>
            <!-- Select Size-->
            <div v-show="$parent.optionActive == 'size'" :class="`flex flex-col basis-12/12 w-full`"
              style="display: flex; justify-content: center">
              <div class="flex flex-row mb-1" v-for="sizeRow in $parent.sizesObj()" :key="sizeRow"
                style="display: flex; justify-content: start">
                <div class="basis-4/12 pr-1" v-for="size in sizeRow" :key="size">
                  <button
                    :class="`${$parent.isActiveSize(size) ? 'cbe-bg-green-lightest' : 'bg-gray-100 hover:bg-gray-200'} bg-gray-100 cbe-btn-text-green cbe-bnt-text-font py-6 w-full rounded-none`"
                    @click="$parent.activeSizeUpdate(size)">
                    {{ size }}
                  </button>
                </div>
              </div>
